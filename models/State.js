...
return db.query("select gamesPlayed, score from states where id=?",[id],callback);
 },
 
 getStateByNumber:function(n,u,callback){
    return db.query("select gamesPlayed, score from states where gamesPlayed=? and user_id = ? ",[n, u],callback);
     },
getLastState:function(u,callback){
    return db.query("select gamesPlayed, score from states where user_id = ? order by gamesPlayed desc limit 1",[u],callback);
    },
     ...
     //db.query("select * from states where user_id = ? order by states.gamesPlayed desc",[s.user_id], function(error, result) {
      //  if(error)
        ...
        db.query("Insert into states (id,gamesPlayed,score,user_id) values(?,?,?,?)",[s.id,s.gamesPlayed,s.score, s.user_id],function(error2,count){
            ...
            return db.query("select gamesPlayed, score from states where id = ?",[s.id],callback);
        });        
    //});
 },
